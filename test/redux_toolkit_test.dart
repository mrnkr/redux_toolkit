    ...
    group('Create reducer', () {
      ...
      setUp(() {
        reducer = createReducer<TestState>(initialState, (builder) {
          builder
            .addCase<RequestItemsAction>((state, action) => state.copyWith(isLoading: true))
            .addCase<FetchItemsSuccessfulAction>((state, action) => state.copyWith(isLoading: false, items: action.payload))
            .addCase<FetchItemsErrorAction>((state, action) => state.copyWith(isLoading: false, error: action.error));
        });
      });
      ...
      test('should yield a non loading state (FetchItemsSuccessfulAction)', () {
        ...
        final result = reducer(prevState, FetchItemsSuccessfulAction(
          payload: items,
        ));
      ...
      test('should yield a non loading state, (FetchItemsErrorAction)', () {
        ...
        final prevState = initialState.copyWith(isLoading: true);
        final result = reducer(prevState, FetchItemsErrorAction(
          error: error,
        ));
